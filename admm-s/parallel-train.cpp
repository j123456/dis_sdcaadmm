...
void exit_with_help()
{
    ...
    "options:\n"
    "-f (0.5) each machine using first (0.5) of its local data at the first iteration\n"
    "-t (1.5) the samples used are increased by (1.5) times at the subsequent iterations\n"
    "-s type : set type of solver (default 1)\n"
    "    1 -- L2-loss support vector machines using ADMM + dual coordinate descent\n"    
    "    2 -- L2-loss support vector machines using ADMM + trusted region Newton method \n"    
    "    3 -- L1 hinger loss support vector machines using ADMM + dual coordinate descent\n"    
    "-c cost : set the parameter C (default 1)\n"
    "-e epsilon : set tolerance of termination criterion for inner iteration (default -s 1: 0.001, -s 2: 0.01)\n"    
    "-M inner_max_iter : set the maximal number of iterations in solving each sub-problem (default 50)\n"
    "-m max_iter : set the maximal number of ADMM iterations (default 50)\n"
    "-B bias : if bias >= 0, instance x becomes [x; bias]; if < 0, no bias term added (default -1)\n"
    "-wi weight : weights adjust the parameter C of different classes (see README for details)\n"
    "-v n : n-fold cross validation mode\n"
    "-R epsilon: relative tolerance for ADMM stopping criteria (default 0.0001)\n"
    "-A epsilon: absolute tolerance for ADMM stopping criteria (default 0.0001)\n"   
    "-r rho: the step size for ADMM (default 1)\n"
    "-x relaxation:  1: yes, 0: no (default: 0)\n"
    "-l relaxation alpha: -l alpha (default: 1.6)\n"    
    "-i intermediate_result_file : the file stores w for each outer iterations in the file \n"
    "-q set inner quiet: 1: make the inner iteration mute, 0: show the inner interation (default 1)\n"        
    "-p show primal objective:    1 -- yes , 0 -- no (default: 0)\n"
    "-n normalize the feature vector:    1 -- yes , 0 -- no (default: 0)\n"
    "-L learning rate for PSGD:    (default: 1E-3)\n"
    "-d decay learning rate for PSGD: 1 -- yes, 0 -- no    (default: 0)\n"
    "-F PSGD more average: 1 --yes, 0 -- no (default: 0) \n"    
    "-f fraction of data used at the first iteration (default: 1) \n"    
    "-t the multiple of sample size between successive iterations (default: 1) \n"    
    );
    exit(1);
}
