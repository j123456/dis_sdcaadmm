...
void psdca(
	const problem *prob, double *w, double *aux_z, double *dnorm,
	double *alpha,
...
/*
		delta = yy - wx - 0.5*alpha[j])/(dnorm[j]*coeff +0.5);
*/

		delta = yy*fmax( 0, fmin(1, (1 - wx*yy)/(dnorm[j]*coeff) + alpha[j]*yy  )  ) - alpha[j];
		alpha[j]  += delta;   
		...
		while (xi->index != -1)
		{
			w[xi->index-1] += coeff*delta*xi->value;
			aux_z[xi->index-1] += coeff*delta*xi->value;			
			xi++;
		}
	}
