...
%pure   = csvread('ocr_ori.txt');
%tw_1 = csvread('./repeat/ocr_tw2.txt');
%tw_1 = csvread('ocr_tw.txt');
%icdm = csvread('ocr_icdm.txt');
pure   = csvread('./repeat/ocr_orif.txt');
tw_1 = csvread('./repeat/ocr_twr.txt');
icdm = csvread('./repeat/ocr_icdmf.txt');
psd = csvread('./repeat/ocr_psd.txt');
...
options.xlimits = [ 40 900];
options.ylimits = [ 75 high];
xx{1} = pure(:,1)';
xx{2} = tw_1(:,1)';
xx{3} = icdm(:,1)';
xx{4} = psd(:,1)';
...
print('ocr_ts_acc','-depsc2','-r300');
system('gs -o -q -sDEVICE=png256 -dEPSCrop -r300 -o ocr_ts_acc.png ocr_ts_acc.eps');
...
prettyPlot( xx, yy, options);
print('ocr_tr_acc','-depsc2','-r300');
system('gs -o -q -sDEVICE=png256 -dEPSCrop -r300 -o ocr_tr_acc.png ocr_tr_acc.eps');
